"use client"
import IconLove from '@/components/IconLove';
  ...
  return (
    ...
    :
     data.length>0 && data?.map(item=>(
         <div className="lg:w-1/4 md:w-1/3 sm:w-1/2 w-full h-[29vh]  flex flex-col items-center justify-center border-r-2  border-b-2 border-red-500 group ">
             <div className="flex-1 mt-8">

             <div className="w-[100px] h-[100px] relative">
                     <Image src={"/p1.jpg"} fill alt='' className='rounded-full object-cover'/>
                    {/* <IconLove onClick={()=>handleSave(item.id)} id={item.id} /> */}
                     <FaHeart id={item.id} onClick={()=>handleSave(item.id)}  className='absolute -top-4 -right-20 fill-red-600'/>
             </div>
             </div>
